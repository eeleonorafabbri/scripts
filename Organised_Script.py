...
def rotation_3d(alpha,beta,x1,y1,z1):
    alpha = math.radians(alpha)
    beta = math.radians(beta)
    x2 = x1*(math.cos(beta)) - y1*(math.sin(beta))
    y2 = x1*(math.sin(beta)) + y1*(math.cos(beta))
    z2 = z1
    x3 = x2
...
cell_feat_orient_df[list(orientation_df.columns)] = pd.DataFrame([[np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]],index=cell_feat_orient_df.index)

for cell_id in orient_id:                                                                       
    id_row = orientation_df.loc[orientation_df['specimen_id']==cell_id, list(orientation_df.columns)]                             
...
cell_feat_orient_new_df = cell_feat_orient_df.dropna(subset=['soma_distance_from_pia'])  #It's the dataframe that contains only cata of cells whose soma_distance_from_pia is not nan
...
# for cell_id in orient_id:
#     # id_row = cell_feat_orient_new_df.loc[cell_feat_orient_new_df['specimen_id']==cell_id]                             
#     # specimen__id = id_row['specimen_id'].values 
#     row_index = cell_feat_orient_new_df[cell_feat_orient_new_df["specimen_id"].values == cell_id].index  
#     upright_angle = cell_feat_orient_new_df.loc[row_index,'upright_angle'].values
#     csl_x = cell_feat_orient_new_df.loc[row_index,'csl__x']
#     csl_y = cell_feat_orient_new_df.loc[row_index,'csl__y']
#     x_coord, y_coord = correct_slice_angle(upright_angle,csl_x,csl_y)
...
for d_type, color in [ [2, axon_color], [3, bas_dendrite_color], [4, api_dendrite_color] ]:
...
#This is the plot of cortical depth (of a single neuron)
fig,ax=plt.subplots(1,1)
...
fig, ax = plt.subplots(1,1)
col_dict = { '1' : 'r' , '2': '#ff7f0e' , '2/3' : 'y' , '3' : 'g' , '4' : 'c' , '5' : 'b' , '6' : '#9467bd' , '6a' : '#e377c2' , '6b' : '#7f7f7f' }
soma_y_coord_vec = []
...
fig,ax=plt.subplots(1,1)
plt.hist(cell_feat_orient_new_df.soma_distance_from_pia, orientation="horizontal", color = 'm')
ax.invert_yaxis()
plt.ylabel("depth")
plt.xlabel("number")
#plt.show()
...
# for d_type, color in [ [2, axon_color], [3, bas_dendrite_color], [4, api_dendrite_color] ]:
#      df = morph_df[morph_df['type'] == d_type]
#      x_coord, y_coord = correct_slice_angle(upright_angle,df['x'],df['y'])
#      y_real, z_real = correct_slice_angle(alpha, y_coord, df['z'])
#      ax.scatter(x_coord,y_real, color=color)
# # ax.invert_yaxis()
# plt.ylabel("y")
# plt.xlabel("x")
# plt.legend(['axons','basal dendrites','apical dendrites'])
...
# for d_type, color in [ [2, axon_color], [3, bas_dendrite_color], [4, api_dendrite_color] ]:
#      df = morph_df[morph_df['type'] == d_type]
#      x_real, y_coord = correct_slice_angle(upright_angle,df['x'],df['y'])
#      y_real, z_real = correct_slice_angle(alpha,y_coord,df['z'])
#      ax.scatter(x_real,y_real, z_real, color=color)
# # ax.invert_yaxis()
# plt.show()
...
# for d_type, color in [ [2, axon_color], [3, bas_dendrite_color], [4, api_dendrite_color] ]:
#      df = morph_df[morph_df['type'] == d_type]
#      ax.scatter(df['x'],df['y'], df['z'], color=color)
# # ax.invert_yaxis()
# plt.show()
...
for d_type, color in [ [2, axon_color], [3, bas_dendrite_color], [4, api_dendrite_color] ]:
     df = morph_df[morph_df['type'] == d_type]
     x_coord, y_coord, z_coord = rotation_3d(slice_angle, upright_angle, df['x'], df['y'], df['z'])
...
# # Plot a histogram of the x, y and z values in one figure for all the cells
# fig, ax = plt.subplots(1,3)
# ax[0].hist(morph_df.x, bins=6, orientation="horizontal", color = 'm')
# ax[0].invert_yaxis()
# ax[0].set_ylabel("x")
# ax[0].set_xlabel("number")
# ax[1].hist(morph_df.y, bins=6, orientation="horizontal", color = 'm')
# ax[1].invert_yaxis()
# ax[1].set_ylabel("y")
# ax[1].set_xlabel("number")
# ax[2].hist(morph_df.z, bins=6, orientation="horizontal", color = 'm')
# ax[2].invert_yaxis()
# ax[2].set_ylabel("z")
